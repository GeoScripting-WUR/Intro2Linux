...
subtitle: "Week 1, Lesson 3: Getting started with Linux (OS-GEOLive): Using the terminal and basic bash scripting"
...
Time  | Activity
------------- | -------------
Morning  |  Self-study: go through the following tutorial, and get confortable with osgeo
1330 to 1430  | Presentation by Allard de Wit (linux), demo session by Erik van Schaik (Github education and Amazon AWS)
...
Linux environment (Lubuntu) with a wide range of free and open-source GIS and Remote sensing tools installed. We are using OS-GEO-live here, especially since installing open-source tools and python modules is so much easiers on Linux then it is on for example another operating system like Windows or Mac.

## Getting started

First open on your computer, VMware workstation Player (as demonstrated in yesterdays lesson).
Second, login into your linux and open Rstudio, and also QGIS.
Third, follow and do a little demo excercise with QGIS available at:

Question 1 (submit via BlackBoard):
...
Typing "man man" in the **terminal** will bring up the manual entry for the man command, which is a good place to start! You can launch the terminal via: **Keyboard Shortcut**: `Ctrl + Alt + T` (see below)
...
- ***Do you know who Frank Warmerdam is? Check via your favorite search engine*** 
...
Do not forget to add a teamname and date at the top of your script.

Hint: http://www.geos.ed.ac.uk/~smudd/TopoTutorials/html/tutorial_raster_conversion.html
