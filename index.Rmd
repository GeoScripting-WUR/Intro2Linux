...
There you go. Why bother looking at your built-in calendar in the clock, when you can fire up your terminal and type `date`, and see what day it is! Just kidding, it’s a simple command, the more useful/difficult ones are coming up next.
...
There are also some shortcuts available. If you type only `cd`, without the path following it, your terminal will change your working directory (whatever it may be) to your `/home/username` directory. Likewise, if you type `cd ~user_name`, it will lead you to the home directory of the specific user you stated.

Try it out yourself. Navigate to a different directory using `cd`, and when there, type `pwd` to see if everything worked as it is supposed to. Once you're done, you should have something like this displayed in your terminal (`cd ..` moves you to the parent directory (one level above your current directory), the ~ represents your home directory):
...
Additionally, you can provide the output of a command to another command using the pipe
operator `|`. In the following, we use the command `head` to only print the first 10
lines of the output:
...
`less` is a command which will display a text file and let you scroll through it. For example, you’re looking for text file `os-release` in `/etc`. You have successfully found it there with `ls /etc`, and now you want to read it. You just use `less /etc/os-release`.
...
- `cp`: copy files e.g. for backing up things or just copying. We will use these commands in the scripts below.
...
Starting and stopping R from the terminal (this is the same as the R console you know from *Positron*/*RStudio*/*RKWard*):
```{r, engine='bash', eval=FALSE}
R # just type R and then q() to exit
q() # answer whether to save the workspace image (y) or not (n)
```
...
So let's get started. First, create a simple text file. While `gedit`, the package we just installed, is used as the main example for editing files, you can also use `positron`, `rstudio` or `rkward` as an alternative text editor if you prefer. In fact, `positron` makes it rather convenient to edit Bash scripts, exactly the same way as R and Python scripts, including the ability to run commands line by line (Ctrl+Enter). It is also worth noting that there are even command-line text editors, like `nano`, which are useful for editing files that require administrative privileges.

Call the new file `HelloWorld.sh`, save this in the `Bash` directory you just created.
Paste the following piece of code into an editor you choose and save it:
...
The first is to set the correct file permissions.
We do this with the `chmod` (change mode) command in the terminal as follows, which needs to be done only once per file (you might have to change the directory first, or specify the file's relative path):
...
```{block, type="alert alert-info"}
**Note**: optionally, we can also run Bash code from R using the `system()` function that can invoke an OS command:
```
...
```{block, type="alert alert-info"}
**Note**: In this lesson, to keep things simple, we'll use `gedit` and Positron as text editors to edit scripts, and run them only from the terminal.
```
...
```{block, type="alert alert-success"}
> **Question 6**: Try it out yourself, and try to do a calculation of e.g. a + b as input variables. Hint: [Shell-tips](https://www.shell-tips.com/2010/06/14/performing-math-calculation-in-bash/)
```
...
GDAL recently introduced a unified command-line interface, `gdal`, which consolidates many
separate utilities under one command with subcommands.
One of the easiest and most useful subcommands for working with raster data is `gdal raster info`. When given an image as an argument, it retrieves and prints all relevant information that is known about the file. This is especially useful if the image contains additional tag data, as is the case with `TIF` files.

```{r, eval = FALSE, engine='bash'}
cd data
ls *.tif
gdal raster info --no-md --no-ct LE71700552001036SGS00_SR_Gewata_INT1U.tif
```
...
Now let's calculate the [NDVI](https://gisgeography.com/ndvi-normalized-difference-vegetation-index/) by running the following command line by line in the terminal. The calculation is done via the subcommand `gdal raster calc`.
See [gdal raster calc](https://gdal.org/en/stable/programs/gdal_raster_calc.html) for more information.
...
ls
echo "* now calculate NDVI using gdal raster calc"
gdal raster calc -i A=input.tif --calc="(A[4]-A[3])/(A[4]+A[3])" --datatype Float32 --output ndvi.tif
...
echo "calculate ndvi"
gdal raster calc -i A=$fn --calc="(A[4]-A[3])/(A[4]+A[3])" --datatype Float32 --output $outfn
echo "look at some histogram statistics"
gdal raster info --stats --hist $outfn
```

```{block, type="alert alert-info"}
More info here on the power of GDAL via the terminal:
[GDAL website](https://www.gdal.org/) and the new unified [`gdal`](https://gdal.org/en/stable/programs/gdal.html) command.

Handy functions are (See the examples at the bottom):

- [`gdal raster info`](https://gdal.org/en/stable/programs/gdal_raster_info.html) — inspect raster metadata (replaces `gdalinfo`)
- [`gdal raster convert`](https://gdal.org/en/stable/programs/gdal_raster_convert.html) — format/type conversion (replaces `gdal_translate`)
- [`gdal raster reproject`](https://gdal.org/en/stable/programs/gdal_raster_reproject.html) — reprojection/resampling (replaces `gdalwarp`)
