...
pagetitle: "Week 1, Lesson 2: Getting started with Linux, the terminal and Bash scripting"
output:
...
# Week 1, Lesson 2: Getting started with Linux, the terminal and Bash scripting
...
There is a large variety of different Linux distributions. One that is handy for geo-information science work is OSGeo-Live, which is also an Ubuntu-based distribution that has a wide range of free and open-source GIS and Remote sensing tools preinstalled.
...
**[See the tutorial on setting up VMWare Horizon on this page.](vmware_horizon_tutorial.html)**

```{block, type="alert alert-danger"}
**Notice**: Make sure you **read the page linked above** and have no problems logging into and using the VDI. From here on out, we will try to work from within the VDI.
```

```{block, type="alert alert-info"}
**In case you can't get the VDI running successfully** (and **only** in that case, so hopefully you don't need to do this!), we have two more alternatives:
...
**The VDIs are recommended**. If you go for one of the alternatives, the systems need to be set up manually.
...
If you like, you can install a Linux distribution on your own laptops or external hard drives. Compared to virtualisation, it is much faster to run a Linux distribution directly. See a [theoretical overview of running Linux on your own hardware](installation.html) and [Linux system setup](../system_setup/index.html).
```
...
So far we have been using only a few of the functions that the commands offer. You can find a list of them by looking at their manual pages by following the last part of the tutorial: [https://news.opensuse.org/2014/07/15/command-line-tuesdays-part-five/](https://news.opensuse.org/2014/07/15/command-line-tuesdays-part-five/)
...
- `sudo apt update`: Update one or more packages
- `sudo apt remove`: Uninstall a package
...
Now let's calculate the NDVI via GDAL terminal by using the `gdal_calc.py` script. See [GDAL_calc](http://www.gdal.org/gdal_calc.html) for more information.
...
If you are ready and want the learn more about using GDAL from the terminal, see [here](http://wiki.americaview.org/display/miview/Introduction+to+GDAL) for another tutorial.
...
Write a shell script that processes Landsat data as provided in the Intro to Raster lesson ([link](https://github.com/GeoScripting-WUR/IntroToRaster/raw/gh-pages/data/gewata.zip)). Specifically, the script should:

1. Take a URL as an argument. The URL links to a ZIP file containing Landsat bands. So the script should run when called as e.g. `./ProcessLandsat.sh https://github.com/GeoScripting-WUR/IntroToRaster/raw/gh-pages/data/gewata.zip`. The script should download and extract the ZIP file into a subdirectory called `data` for further processing.
2. Derive NDVI from the Landsat bands, resample it to pixels of 60 m, reproject to WGS84 (EPSG:4326) and put the output into a subdirectory called `output`.
3. Be reproducible and rerunnable: no use of `cd` and running the script twice still gives correct output. The script leaves only one output file, no intermediate ones. If a filename is reused in more than one place, a variable should be used instead of writing the name twice.

```{block, type="alert alert-info"}
**Tips**: make use of Bash variables such as `$1`, utilities like `mkdir`, `wget`, `unzip`, `rm`, `gdalwarp` and `gdal_calc.py`. Check their `man` pages for arguments.
```

For **bonus points**: find a way to resample and reproject the file *without writing a temporary TIFF file to disk*, ideally with a single line of code. **Tips**: `mkfifo` and `|`, or [process substitution](https://en.wikipedia.org/wiki/Process_substitution) and `/proc/self/fd/0` as input file name.
...
- [http://www.howtogeek.com/140679/beginner-geek-how-to-start-using-the-linux-terminal/](http://www.howtogeek.com/140679/beginner-geek-how-to-start-using-the-linux-terminal/)
...
- [http://www.cyberciti.biz/faq/pwd-linux-unix-command-examples/](http://www.cyberciti.biz/faq/pwd-linux-unix-command-examples/)
