...
*Linux* is a free and open-source operating system kernel. In this course we will be working with a customised *Xubuntu*, which is a popular lightweight *Linux distribution*. A Linux distribution is an operating system that is built upon the Linux kernel and includes plenty of handy programs for daily work.
...
During the course we will work in a Linux environment. There are several choices, listed in the order of most to least tested for the course:

1. VMware Horizon virtual desktop infrastructure (VDI) that is provided by the WUR IT department. It is available over the internet for each student in the course and supported by the IT department.
2. Local VirtualBox VMs installed on the PCs in the PC labs. These can only be accessed from a specific PC and intended as a fallback is there are problems when using VDIs.
3. Your own hardware: if you know what you are doing, you can install Linux on your own laptops or external hard disks.
...
If you like, you can install a Linux distribution on your own laptops or external hard drives. Compared to virtualisation, it is much faster to run a Linux distribution directly. See [this page](installation.html) for tips on that.
