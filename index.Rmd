...
First, start *Git Bash*. This will open a terminal window. In it, enter: `ssh -NL 2049:localhost:2049 ubuntu@<ip>` where `<ip>` is the IP address of your VM. This will establish a secure SSH connection to the VM on port 2049. You can use this method for other applications and ports as well, such as to access RStudio Server. When you are done, to disconnect the secure connection, press Ctrl+C in the Git Bash window.

To map the network drive, right-click on your desktop, choose *New* → *Shortcut*, and enter `<ip>:/home/ubuntu/geoscripting` (again, use the IP of your VM). Double-clicking on the shortcut, you will see an empty folder. It is the */home/ubuntu/geoscripting* directory on the VM, and you can drag and drop files and folders to and from it.
