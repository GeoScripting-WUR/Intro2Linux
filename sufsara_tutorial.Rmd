...
<!-- Navigate to your [SRAM profile page](https://sram.surf.nl/profile). Generate an SSH key on your own machine. Paste your *public* SSH key into the text box, and click the "Save" button. You should never paste the *private* key anywhere (it will also not be accepted if you try). -->

<!-- Please refer to online tutorials to find/create an SSH key for your machine, like [this one](https://www.howtogeek.com/762863/how-to-generate-ssh-keys-in-windows-10-and-windows-11/) for Windows. -->

<!-- **Hint**: another easy way to find your SSH key is by running Git GUI on your own machine, through Help → Show SSH Key.  -->

## Logging into the VM

There are two ways to log into your virtual machine (aside from the in-browser option). The first option is via the "Remote Desktop" app that is built-into Windows, or an RDP client on the other operating systems. The second option is through a command line using *Git Bash*. 

To connect to the VM using the **Remote Desktop** app ('Extern Bureaublad' if you have Dutch language settings), you simply need to specify the IP address of the VM to connect (and potentially dismiss some warnings about missing certificates). You can find the IP address of your VM by pressing the *down* arrow next to 'State: running' on [this page](https://portal.live.surfresearchcloud.nl/). For the next steps, it is the same as the in-browser option.
